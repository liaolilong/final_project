package com.wlaq.educenter.controller;
...
import com.wlaq.educenter.utils.ConstantWxUtils;
import com.wlaq.educenter.utils.HttpClientUtils;
...
public class WxApiController {
    ...
    public String callback(String code,String state){
        try{
            //1获取code值，临时票据，类似于验证码

            //2拿着code请求微信固定的地址，得到两个值 access_token和openid
            ...
            //把扫码人信息添加数据库里面
            //判断数据库表里面是否存在相同数据，根据openid判断
            UcenterMember member = memberService.getOpenIdMember(openid);
            if(member == null){
                //member是空，表没有相同微信数据，进行添加

                //3拿着得到access_token和openid，再去请求微信提供的固定地址，获取到扫码人信息
                //访问微信的资源服务器，获取用户信息
            ...
            throw new wlaqException(20001,"登录失败");
        }
