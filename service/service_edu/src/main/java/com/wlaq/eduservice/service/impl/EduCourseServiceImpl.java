...
import com.wlaq.eduservice.service.EduCourseDescriptionService;
...
public class EduCourseServiceImpl extends ServiceImpl<EduCourseMapper, EduCourse> implements EduCourseService {
    ...
    public String saveCourseInfo(CourseInfoVo courseInfoVo) {
        ...
        if(insert ==0){
            //添加失败
            throw new wlaqException(20001,"添加课程信息失败");
        }
    ...
    public void updateCourseInfo(CourseInfoVo courseInfoVo) {
        ...
        if(update == 0){
            throw new wlaqException(20001,"修改课程信息失败");
        }
        //修改描述表
    ...
    public void removeCourse(String courseId) {
            ...
            if(result == 0){//失败返回
                throw new wlaqException(20001,"删除失败");
    ...
    public Map<String, Object> getCourseFrontList(Page<EduCourse> pageParam, CourseFrontVo courseFrontVo) {
        ...
        //判断条件值是否为空，不为空拼接
        if(!StringUtils.isEmpty(courseFrontVo.getSubjectParentId())){//一级分类是否存在
            wrapper.eq("subject_parent_id",courseFrontVo.getSubjectParentId());
        }
        if(!StringUtils.isEmpty(courseFrontVo.getSubjectId())){//二级分类是否存在
